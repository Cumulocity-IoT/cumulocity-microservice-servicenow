            ...
            cachedUsers.forEach(async (user) => {
                ...
                if (tenantId === user.tenant && '6' === adata.u_state) {
                    ...
                    const {
                        data,
                        res
                    } = await client.alarm.update(partialUpdateAlarm);
...
async function getUsers() {
    ...
    const client = new FetchClient(new BasicAuth({
        tenant,
        user,
        password
    }), baseUrl);
...
function listen2NewAlarms() {
    ...
    cachedUsers.forEach(async (user) => {
        ...
        let {
            data
        } = await client.alarm.list(filter);
        data.forEach((alarm) => {
        ...
        try {
            ...
            for (const user of cachedUsers) {
                ...
                if (process.env.C8Y_BOOTSTRAP_TENANT === user.tenant) {
                    ...
                    getTenantOptions(auth).then((cachedOptions) => {
